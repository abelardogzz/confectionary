...
def p_seen_pn_ratio(p):
    ...
    if result_type == 0 or result_type == 1:
        ...
        quad = generate_Quad("ration",result,"--",memPos)
...
def p_seen_pn_flavor(p):
    ...
    if left_type == 0 or left_type == 1 :
        ...
        quad = generate_Quad("flavor", left_operand, "--", memPos)
...
#------------INT GRID--------------------
def p_igrid(p):
    ''' igrid : INT "[" CTEINT seen_cte "]" "[" CTEINT seen_cte "]" seen_grid ID "=" "{" bigrid "}" ";"'''
    global context
    global incrementalNumber
    global VirtualMem
    ...
    for k, v in DiVars.items():
        if p[11] == v or isinstance(v, list) and p[11] in v:
            varContext = DiVars[k][3]
            if varContext == context:
                addToDict = False 
                repeated_var_error(p[11])

    if addToDict == True:
        aux = GlobalID + str(incrementalNumber)
        DiVars[aux]=[p[11],p[14],7, context]
        print("Check this -> ",VirtualMem[memIndex][0])
        print("And now this -> ",len(DiVars[aux][1]))
        memPos = (VirtualMem[memIndex][0]) - (len(DiVars[aux][1]))
        DiVars[aux] = DiVars[aux] + [memPos] 
        incrementalNumber = incrementalNumber + 1
    #print("INT-GRIDDY")
    pass
...
#------------FLOAT GRID--------------------
def p_fgrid(p):
    ''' fgrid : FLOAT "[" CTEINT seen_cte "]" "[" CTEINT seen_cte "]" seen_grid ID "=" "{" bfgrid "}" ";"'''
    global context
    global incrementalNumber
    addToDict = True
...
#------------STRING GRID--------------------
def p_sgrid(p):
    ''' sgrid : STRING "[" CTEINT seen_cte "]" "[" CTEINT seen_cte "]" seen_grid ID "=" "{" bsgrid "}" ";"'''
    global context
    global incrementalNumber
    addToDict = True
...
#------------INT SHELF-----------
def p_ishelf(p):
    ''' ishelf : INT "[" CTEINT seen_cte "]" ID seen_shelf "=" "{" bishelf "}" ";" '''
    ...
    for k, v in DiVars.items():
          if p[6] == v or isinstance(v, list) and p[6] in v:
              varContext = DiVars[k][3]
              if varContext == context:
                  addToDict = False 
                  repeated_var_error(p[6])

    if addToDict == True:
        aux = GlobalID + str(incrementalNumber)
        DiVars[aux]=[p[6],p[10],4, context]
        incrementalNumber = incrementalNumber + 1
...
#------------FLOAT SHELF-----------
def p_fshelf(p):
    ''' fshelf : FLOAT "[" CTEINT seen_cte "]" ID seen_shelf "=" "{" bfshelf "}" ";"'''
    global context
    global incrementalNumber
    addToDict = True
...
#------------STRING SHELF-----------
def p_sshelf(p):
    ''' sshelf : STRING "[" CTEINT seen_cte "]" ID seen_shelf "=" "{" bsshelf "}" ";"'''
    global context
    global incrementalNumber
    addToDict = True
...
def p_seen_pn_drCU(p):
    ...
    if left_type == 0 or left_type == 1 :
        ...
        availTemp = availTemp + 1
        memPos = AVAIL(context, left_type, 1, 0)
        quad = generate_Quad("drCup", left_operand, "--", memPos)
...
def p_seen_pn_drCA(p):
    ...
    if left_type == 0 or left_type == 1 :
        ...
        quad = generate_Quad("drCane", left_operand, "--", memPos)
        quads = quads + [quad]
    else:
...
def p_seen_pn_drCH(p):
    ...
    if left_type == 0 or left_type == 1 :
        ...
        quad = generate_Quad("drChoc", left_operand, "--", memPos)
        quads = quads + [quad]
    else:
    ...
    if len(POper) != 0:
      if POper[-1] == '=':
          ...
          if result_Type != 99:
              ...
              print("Lo que envio ->", operator, left_operand, "", result)
...
def p_seen_pn_show(p):
    ...
    quad = generate_Quad("show",left_operand,"--",memPos)
    ...
    if     len(POper) != 0:
        if POper[-1] == 5 or POper[-1] == 6 or POper[-1] == 7 or POper[-1] == 8 or POper[-1] == 9 or POper[-1] == 10 or POper[-1] == 11 or POper[-1] == 12:
            ...
            if resultType != 99:
                ...
                availTemp = availTemp + 1
                quad = generate_Quad(operator, left_operand , right_operand , result)
...
def generate_Quad(operator, left_op, right_op, result):
  ...
  """addToDict = True
  #constAlreadyExist = False
  key = 0
  for k, v in DiConst.items():
    if left_op == v or isinstance(v, list) and left_op in v:
      print("mi key es -> ", k)
      addToDict = False
      #constAlreadyExist = True
      key = k
  ...
  if addToDict:
    ...
    aux = GlobalConst + str(incrementalNumberConst)
    incrementalNumberConst = incrementalNumberConst + 1
    DiConst[aux]=[left_op,cteType,memPos]
    key = aux"""

  #left_op = DiConst[key][2]
